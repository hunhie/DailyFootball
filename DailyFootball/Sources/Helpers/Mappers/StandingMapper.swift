...
struct StandingMapper {
  static func mapStanding(from table: StandingTable) -> Standing {
  ...
  static func mapStandings(from tables: List<StandingTable>) -> [Standing] {
    return tables.map { table in
      let teamTable = table.team!
      let all = table.all!
      let home = table.home!
      let away = table.away!
      return mapStanding(from: table)
    }
  }
}
